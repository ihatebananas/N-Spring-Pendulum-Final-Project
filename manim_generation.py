...
class SpringPendulum(Scene):
    ...
    def construct(self):
        ...
        N = 5
        zags = 8 + int(np.sqrt(N))
        spr_width = 1/N

        M_vec = np.zeros(N) + 25/N
        K_vec = np.zeros(N) + 250*N
        L_vec = np.zeros(N) + spr_width*5
        spring_pendulum = NSP(M_vec, K_vec, L_vec)

        x_0 = [0 for i in range(1, N+1)]
        y_0=[-i*(spr_width*5) for i in range(1, N+1)] 
        x_dot_0 = np.zeros(N) + 1 + 10/N
        y_dot_0 = np.zeros(N)
        t_pts = np.arange(0, 20, 0.02)

        solution = spring_pendulum.solve_ode(x_0, y_0, x_dot_0, y_dot_0, t_pts)

        pivot = Dot().move_to(3*UP)
        Mass_Vector = [Dot() for i in range(N)]
        Spring_Vector = [VMobject() for i in range(N)]

        x1 = solution[0, :]
        y1 = solution[N, :]
        Mass_Vector[0] = Dot(radius=0.02*M_vec[0]).move_to(x1[0]*RIGHT+(y1[0]+3)*UP).set_color(BLUE)
        ...
        for k in range(1,N):
            xk = solution[k, :]
            yk = solution[k+N, :]
            Mass_Vector[k] = Dot(radius=0.02*M_vec[k]).move_to(xk[0]*RIGHT+(yk[0]+3)*UP).set_color(BLUE)
        ...
        for i in range(len(x1) - 1):
            ...
            self.add(*Mass_Vector)
            animations = [ApplyMethod(Mass_Vector[m].move_to, solution[m, :][i+1]*RIGHT + (solution[m+N, :][i+1]+3)*UP) for m in range(N)]
